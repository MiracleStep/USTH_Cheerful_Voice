...
<script type="text/javascript">
	...
	function getMsg(pageNum){
		$.ajax({
			...
			type : "POST",
			async : "true",
			data : {"action" : "searchMsg", "pageNum": pageNum, "key" : key, "username" : username, "theid" : theid, "state": 1},
			dataType : "json",
			success : function(data) {
				if (data.res==1){
					...
					$.each(data.data.list, function(index, msgItem) {
						...
						msg.find(".category").text(msgItem.category);
						msg.find(".thename").text(msgItem.thename);
						msg.find(".author").text(msgItem.realname);
						msg.find(".time").text(msgItem.msgtime);
						
						msg.find(".delete").attr("onclick", "removeMsg("+msgItem.msgid+")");
						msg.find(".pass").attr("onclick", "passMsg("+msgItem.msgid+")");
	...
	function removeMsg(msgid){
		if(confirm("确认删除吗？")){
			$.ajax({
				url:"/admin/message/removeMsg.action",
				type:"post",
				data:{"action":"removeMsg","msgid" : msgid},
				dataType:"json",
				success:function(data){
					if(data.res==1){
						alert ("删除成功");
	...
	function passMsg(msgid){
		$.ajax({
			url : "/admin/message/passMsg.action",
			type : "post",
			async : "true",
			data : {"action" : "passMsg", "msgid" : msgid},
			dataType : "json",
			success : function(data){
				if (data.res == 1){
					alert ("通过成功");
	...
	<div class="container">		
		...
		<div class="row msglist template">
			...
			<div class="col-sm-9 col-xs-8">
				<div class="col-sm-8 col-xs-8 title"><a class="tit" target="_blank">标题</a></div>
				<div class="col-sm-2 col-xs-6 category">分类</div>
				<div class="col-sm-2 col-xs-6 thename">话题</div>
			...
			<div class="col-sm-2 col-xs-3">
				...
				<button class="btn btn-danger delete">拒绝</button>
			</div>
